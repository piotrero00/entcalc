function output = uppermul(rho,dims,dec,insep,r,iteramax,dif,sepitera)
% uppermul computes the upper bound of the geometric entanglement 
% of the multipartite state rho.
%
% Parameters
% ----------
% rho : matrix
...
%     integer. Number of elements in dims must be at least 3.
%
% dec : integer (0 or 1)
...
%
% - The input state must be 3-partite.
%
...
%    0.3025
    ...
    if length(dims)<3
        error('System should be at least 3-partite');
    end
    ...
    F=42;
    Fp=0;
    for l = 1:iteramax          %Iteration steps
        if abs(F-Fp)<dif
            break
        end
        A = zeros(r);
        ...
        alpha = cell(r,1);      %Updating state decomposition
        for i = 1:r
        ...
        for ity = 1:sepitera
            tester = zeros(r, 1);
            for ij = 1:r 
                ten=Tensor(psiNew{ij});
                pra = phiCell{ij};  % kopia elementu i (jeśli sql jest cell array)
                pre = pra{1};
                for k = 2:length(pra)
                    pre = kron(pre, pra{k});
                end
                for ind=1:length(dims)
                     sea= TensorWithout(phiCell{ij},ind);         %Updating separable decomposition
                     zd=sea'*ten;
                     zd=zd/norm(zd);
                     phiNewCell{ij}{ind}=zd;
                end
                pra = phiNewCell{ij};          % wybór i-tego elementu
                ppo = pra{1};          
                for k = 2:length(pra)
                    ppo = kron(ppo, pra{k});
                end
                overlap_val = abs(pre' * ppo);  % <pre|ppo>, wektory kolumnowe
                if 1 - overlap_val < 10^(-11)
                    tester(ij) = 1;
                end
            end 
            if all(tester == 1)
        ...
        rsq=sqrtm(rin);
        Fp=F;                  %Computing fidelity
        F=1-real(trace(rsq)^(2));
